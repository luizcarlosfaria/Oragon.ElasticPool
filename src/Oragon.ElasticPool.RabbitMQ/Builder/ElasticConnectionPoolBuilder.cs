...
/// <summary>
/// Adapter-side fluent builder for an <c>IElasticPool&lt;IConnection&gt;</c>. Exposes the
...
/// the pool directly with <c>services.AddElasticPool&lt;IConnection&gt;(...)</c> instead of
...
public sealed class ElasticConnectionPoolBuilder
{
    ...
    /// </summary>
    public ElasticConnectionPoolBuilder WithSweepInterval(TimeSpan t)
    {
        if (t <= TimeSpan.Zero)
    ...
    /// </summary>
    public ElasticConnectionPoolBuilder WithShrinkOnUtilizationPercent(double p)
    {
        if (p < 0.0 || p > 1.0)
    ...
    /// Validates <c>p in (0, 1]</c>.
    /// </summary>
    public ElasticConnectionPoolBuilder WithShrinkTargetUtilizationPercent(double p)
    {
        if (p <= 0.0 || p > 1.0)
    ...
    /// Validates <c>n &gt;= 1</c>.
    /// </summary>
    public ElasticConnectionPoolBuilder WithShrinkBatchSize(int n)
    {
        if (n < 1)
    ...
    /// Validates <c>n &gt;= 0</c>.
    /// </summary>
    public ElasticConnectionPoolBuilder WithShrinkCooldownWindows(int n)
    {
        if (n < 0)
