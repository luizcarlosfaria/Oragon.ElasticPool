# Oragon.AdaptivePool.RabbitMQ.Sample.LiveDashboard

Aspire + Blazor dashboard for visually validating `Oragon.AdaptivePool.RabbitMQ`.
...
- `IAdaptivePool<T>.Total`, `Available`, `InUse`, and `Waiting` can be read directly
...
```bash
dotnet run --project samples/Oragon.AdaptivePool.RabbitMQ.Sample.LiveDashboard/Oragon.AdaptivePool.RabbitMQ.Sample.LiveDashboard.AppHost
```
...
- `*.ServiceDefaults` wires Aspire health checks and OpenTelemetry, including the
  `Oragon.AdaptivePool` meter/source.
