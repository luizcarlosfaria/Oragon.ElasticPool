...
public sealed class LiveLoadController(
    [FromKeyedServices(LivePoolNames.PoolName)] IAdaptivePool<IConnection> connectionPool,
    [FromKeyedServices(LivePoolNames.PoolName)] IAdaptivePool<IChannel> channelPool,
    ILogger<LiveLoadController> logger) : BackgroundService
{
    private const int MaxConcurrency = 200;
    private const string Exchange = "oragon.adaptivepool.live";
    private static readonly ReadOnlyMemory<byte> Body = Encoding.UTF8.GetBytes("oragon-adaptivepool-live");
    ...
    private static PoolSnapshot Snapshot<T>(IAdaptivePool<T> pool)
        where T : notnull =>
