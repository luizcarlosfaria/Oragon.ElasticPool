...
```bash
INIT=$(node "/mnt/p/dynamic-pool/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "/mnt/p/dynamic-pool/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "/mnt/p/dynamic-pool/.claude/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
