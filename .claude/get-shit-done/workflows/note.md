...
- **Global scope**: `/mnt/p/dynamic-pool/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/mnt/p/dynamic-pool/.claude/notes/`)
...
2. Glob `/mnt/p/dynamic-pool/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/mnt/p/dynamic-pool/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/mnt/p/dynamic-pool/.claude/notes/` — works in any directory
