...
internal static class HeavyResourceElasticityRunner
{
    ...
    public static async Task RunAsync(string[] args)
    {
        ...
        {
            Console.WriteLine("Usage: dotnet run --project tests/Oragon.AdaptivePool.Core.Benchmarks -c Release -f net10.0 -- elasticity [--profile smoke|readme|real]");
        ...
        Console.WriteLine("Strategies: NoPool, AdaptivePool, Microsoft.Extensions.ObjectPool");
        Console.WriteLine();
    ...
    private static IEnumerable<Func<IResourceStrategy>> CreateStrategyFactories(ElasticityProfile profile)
    {
        ...
        yield return () => new AdaptivePoolStrategy(profile.ResourceMegabytes);
    ...
    private static Dictionary<int, PhaseReport> GetAdaptivePhasesByIndex(IEnumerable<StrategyReport> reports) =>
        reports
            .FirstOrDefault(static report => string.Equals(report.Strategy, "AdaptivePool", StringComparison.Ordinal))
    ...
    {
        var adaptive = reports.FirstOrDefault(static report => string.Equals(report.Strategy, "AdaptivePool", StringComparison.Ordinal));
        ...
        sb.AppendLine();
        sb.AppendLine("## AdaptivePool vs ObjectPool logical retention");
        sb.AppendLine();
        sb.AppendLine("| Phase | Requested req/s | AdaptivePool retained MB | ObjectPool retained MB | ObjectPool excess MB | Retention factor |");
        sb.AppendLine("|---:|---:|---:|---:|---:|---:|");
    ...
    private sealed class AdaptivePoolStrategy : IResourceStrategy
    {
        private readonly IAdaptivePool<HeavyResource> _pool;
        public string Name => "AdaptivePool";
        public HeavyResourceTracker Tracker { get; } = new();

        public AdaptivePoolStrategy(int resourceMegabytes)
        {
            _pool = AdaptiveObjectPoolFactory.Build<HeavyResource>(new ServiceCollection().BuildServiceProvider())
