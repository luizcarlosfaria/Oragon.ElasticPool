...
public class Phase2CountersAndHistogramsTests
{
    ...
    private const string MeterName = "Oragon.AdaptivePool";

    private static (ServiceProvider sp, IAdaptivePool<Resource> pool) BuildKeyed(
        Action<AdaptivePoolBuilder<Resource>> configure)
    {
        ...
        return (sp, sp.GetRequiredKeyedService<IAdaptivePool<Resource>>(PoolName));
    }
    ...
    public async Task ShrinkCount_IncrementsExactlyOncePerShrink()
    {
        ...
        var pool = (AdaptivePool<Resource>)ipool;
        try
        {
            using var collector = new MetricCollector<long>(
