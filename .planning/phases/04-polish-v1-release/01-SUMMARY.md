...
requires:
  ...
  - "Phase 3: samples/Oragon.AdaptivePool.RabbitMQ.Sample.BurstyPublisher (link target)"
...
provides:
  ...
  - "src/Oragon.AdaptivePool.Core/README.md (embedded in Core .nupkg; 30s quickstart, M.E.OP comparison table, OTel snippet, instrument inventory)"
  - "src/Oragon.AdaptivePool.RabbitMQ/README.md (embedded in RabbitMQ .nupkg; layered example + 3 callouts: channel-per-publisher, connection sizing, AutomaticRecoveryEnabled override)"
...
affects:
  ...
  - "src/Oragon.AdaptivePool.Core/Oragon.AdaptivePool.Core.csproj (+4 / -1 lines)"
  - "src/Oragon.AdaptivePool.RabbitMQ/Oragon.AdaptivePool.RabbitMQ.csproj (+4 / -1 lines)"
tech-stack:
  added: []
  patterns:
...
key-files:
  created:
    ...
    - "src/Oragon.AdaptivePool.Core/README.md (170 lines)"
    - "src/Oragon.AdaptivePool.RabbitMQ/README.md (134 lines)"
  modified:
    - "Directory.Build.props (centralised package metadata)"
    - "src/Oragon.AdaptivePool.Core/Oragon.AdaptivePool.Core.csproj (Description / PackageTags refined; README pack item; PackageLicenseExpression removed)"
    - "src/Oragon.AdaptivePool.RabbitMQ/Oragon.AdaptivePool.RabbitMQ.csproj (Description / PackageTags refined; README pack item; PackageLicenseExpression removed)"
decisions:
  ...
  - "READMEs use absolute https://github.com/oragon/Oragon.AdaptivePool/... links to Sample/LICENSE; per-csproj README ships in .nupkg where relative paths can't resolve. Root README uses relative links (samples/..., src/..., CHANGELOG.md, LICENSE) for in-repo navigation on GitHub"
metrics:
  duration: "~30 minutes"
...
Landed all OSS-quality documentation, licensing, and NuGet packaging metadata required to ship `Oragon.AdaptivePool.Core` and `Oragon.AdaptivePool.RabbitMQ` to NuGet.org. After this plan, `dotnet pack -c Release` produces well-formed `.nupkg` files containing per-package README, embedded icon, MIT license expression, refined description/tags/authors/project URLs — and the repo presents a polished GitHub-recognizable face (root README, LICENSE, CHANGELOG). No code changes; metadata + docs only.
...
```
dotnet pack src/Oragon.AdaptivePool.Core/Oragon.AdaptivePool.Core.csproj -c Release
ok Successfully created package '/tmp/pack-core/Oragon.AdaptivePool.Core.0.0.0-alpha.0.79.nupkg' (98.5 KB)
ok Successfully created package '/tmp/pack-core/Oragon.AdaptivePool.Core.0.0.0-alpha.0.79.snupkg' (52.7 KB)

dotnet pack src/Oragon.AdaptivePool.RabbitMQ/Oragon.AdaptivePool.RabbitMQ.csproj -c Release
ok Successfully created package '/tmp/pack-rmq/Oragon.AdaptivePool.RabbitMQ.0.0.0-alpha.0.79.nupkg' (43.0 KB)
ok Successfully created package '/tmp/pack-rmq/Oragon.AdaptivePool.RabbitMQ.0.0.0-alpha.0.79.snupkg' (36.2 KB)
```

### Package contents (unzip -l)

**Core .nupkg** contains:
- `lib/net8.0/Oragon.AdaptivePool.Core.dll` (75776 bytes)
- `lib/net9.0/Oragon.AdaptivePool.Core.dll` (75776 bytes)
- `lib/net10.0/Oragon.AdaptivePool.Core.dll` (75776 bytes)
- `icon.png` (258 bytes)
- `README.md` (8263 bytes — the per-package one)

**RabbitMQ .nupkg** contains:
- `lib/net8.0/Oragon.AdaptivePool.RabbitMQ.dll` (31232 bytes)
- `lib/net9.0/Oragon.AdaptivePool.RabbitMQ.dll` (31232 bytes)
- `lib/net10.0/Oragon.AdaptivePool.RabbitMQ.dll` (31232 bytes)
- `icon.png` (258 bytes)
...
- README.md (root) starts with `# Oragon.AdaptivePool` and references `samples/Oragon.AdaptivePool.RabbitMQ.Sample.BurstyPublisher` ✓
...
- Core README contains `^# Oragon.AdaptivePool.Core`, `AddAdaptivePool<`, `Why not \`Microsoft.Extensions.ObjectPool`, `AddMeter("Oragon.AdaptivePool")`, `AddSource("Oragon.AdaptivePool")`, sample link ✓
- RabbitMQ README contains `^# Oragon.AdaptivePool.RabbitMQ`, `AddAdaptiveConnectionPool`, `AddAdaptiveChannelPool`, `AutomaticRecoveryEnabled`, the connection-pool sizing callout ("connection pool's `MaxSize`"), sample link ✓
...
- **Fix:** `rm -rf src/Oragon.AdaptivePool.{Core,RabbitMQ}/obj src/Oragon.AdaptivePool.{Core,RabbitMQ}/bin` and `dotnet restore --no-cache --force` regenerated clean assets without the Windows fallback. Subsequent pack invocations succeeded.
...
2. **GitHub remote URL assumption:** Both root README badges and per-package README absolute links assume `oragon/Oragon.AdaptivePool` as the canonical GitHub remote. Plan 02 must verify (or update) this when configuring `release.yml`'s `dotnet nuget push` step. If the remote slug differs, update: Directory.Build.props `RepositoryUrl`/`PackageProjectUrl`, root `README.md` badge URLs, and the absolute links inside both per-package READMEs.
...
- `README.md` ✓ FOUND
- `src/Oragon.AdaptivePool.Core/README.md` ✓ FOUND
- `src/Oragon.AdaptivePool.RabbitMQ/README.md` ✓ FOUND
- `Directory.Build.props` ✓ MODIFIED (Task 1)
- `src/Oragon.AdaptivePool.Core/Oragon.AdaptivePool.Core.csproj` ✓ MODIFIED (Task 2)
- `src/Oragon.AdaptivePool.RabbitMQ/Oragon.AdaptivePool.RabbitMQ.csproj` ✓ MODIFIED (Task 2)
...
- `/tmp/pack-core/Oragon.AdaptivePool.Core.0.0.0-alpha.0.79.nupkg` ✓ produced
- `/tmp/pack-core/Oragon.AdaptivePool.Core.0.0.0-alpha.0.79.snupkg` ✓ produced
- `/tmp/pack-rmq/Oragon.AdaptivePool.RabbitMQ.0.0.0-alpha.0.79.nupkg` ✓ produced
- `/tmp/pack-rmq/Oragon.AdaptivePool.RabbitMQ.0.0.0-alpha.0.79.snupkg` ✓ produced
