...
`CS1061 'ElasticPoolBuilder<MyClient>' does not contain a definition for 'MinSize'`.
...
`GetRequiredService<IElasticPool<T>>()` instead of the actual `GetRequiredKeyedService`
because `AddElasticPool` registers a keyed singleton.
