...
provides:
  - 12 public types under Oragon.AdaptivePool.Core (IAdaptivePool<T>, IPoolItem<T>, IItemFailurePolicy<T>, FactoryDelegate<T>/BeforeUseDelegate<T>/CheckDelegate<T>/AfterUseDelegate<T>/ReleaseDelegate<T>, AdaptivePoolBuilder<T>, AdaptivePoolOptions<T>, AdaptiveObjectPoolFactory, WaitBehavior, PoolState, FailureKind, FailureDecision, DiscardAndReplaceFailurePolicy<T>, PoolExhaustedException, ServiceCollectionExtensions.AddAdaptivePool<T>)
  - sealed internal AdaptivePool<T> engine implementing direct-handoff Channel<TaskCompletionSource<PoolEntry<T>>> waiter, Interlocked counter rollback, dual IDisposable+IAsyncDisposable with drain semantics, eager warm-up via ReadyAsync()
  - DI extension `services.AddAdaptivePool<T>(name, configure)` registering keyed singletons + non-keyed fallback for the default (string.Empty) name
  - Telemetry: Meter "Oragon.AdaptivePool" via IMeterFactory (with `new Meter` fallback) + counters `pool.acquire.count` & `pool.factory.failures` tagged by `pool.name`
...
decisions:
  ...
  - AdaptivePool<T> engine is internal sealed; consumers obtain it strictly via the IAdaptivePool<T> contract returned by AdaptivePoolBuilder<T>.Build().
  ...
  - AdaptivePool<T> uses `object` as lock target (none currently — engine is lock-free via Interlocked + Channel) but the policy stands: never use `System.Threading.Lock` (net9+) so we keep a single shared internal model across all 3 TFMs (per RESEARCH "Single instance lock target").
...
**One-liner:** Implemented the entire Phase 1 surface for `Oragon.AdaptivePool.Core` — 12 public types (interfaces, hook delegates, builder, options, default failure policy, exception, enums, DI extension) plus the sealed internal `AdaptivePool<T>` engine using `Channel<TaskCompletionSource<PoolEntry<T>>>` direct-handoff waiter, Interlocked counter rollback, dual `IDisposable`+`IAsyncDisposable` drain, eager warm-up via `ReadyAsync()`, telemetry through `IMeterFactory` (with `new Meter` fallback), source-gen logging via `[LoggerMessage]`, and a DI extension that registers keyed singletons with named-options + non-keyed fallback for the default name. `dotnet build Oragon.AdaptivePool.sln -c Release` is fully green with `TreatWarningsAsErrors=true`; `dotnet test` runs the Plan 01 placeholder smoke test on net8/net9/net10 — all 3 pass.
...
| File | Type | Purpose |
| --- | --- | --- |
| `Abstractions/IAdaptivePool.cs` | interface | `IAdaptivePool<T> : IDisposable, IAsyncDisposable` — read-only `MaxSize`/`MinSize`/`Available`/`InUse`, sync `Acquire()` (throws when empty), async `AcquireAsync(CancellationToken)`, `ReadyAsync()` for warm-up await |
...
| `Builder/AdaptivePoolOptions.cs` | sealed record | Frozen options carrier with `required init` Factory/MinSize/MaxSize/InitialSize/FailurePolicy + optional BeforeUse/Check/AfterUse/Release/WhenExhausted/TimeProvider/PoolName |
| `Builder/AdaptivePoolBuilder.cs` | sealed class | Fluent builder; `.Build()` validates `0 ≤ Min ≤ Initial ≤ Max`, requires `Factory`, defaults FailurePolicy to `DiscardAndReplaceFailurePolicy<T>`. Also has `internal WithName(string)` for DI extension stamping. |
| `Builder/AdaptiveObjectPoolFactory.cs` | static class | Public entry point: `Build<T>(IServiceProvider, CancellationToken) -> AdaptivePoolBuilder<T>` |
...
- `AddAdaptivePool<T>(this IServiceCollection, string name, Action<AdaptivePoolBuilder<T>> configure)` — single overload, name required, may be `string.Empty`.
- Internal `AdaptivePoolBuilderConfigurator<T>` carries the user's configure delegate via named `IOptions`.
- `TryAddKeyedSingleton<IAdaptivePool<T>>(name, factory)` builds the pool lazily on first resolve. The factory: pulls the configurator from `IOptionsMonitor<T>.Get(keyName)`, probes for `IHostApplicationLifetime.ApplicationStopping` via reflection (no Hosting hard dep), instantiates a builder via `AdaptiveObjectPoolFactory.Build<T>(sp, lifetimeCt)`, calls `builder.WithName(keyName)` (internal-same-assembly call — no reflection sidedoor needed; the plan's reflection variant was the fallback path), invokes the user's configurator, and finally `.Build()`s.
- When `name == string.Empty`, also `TryAddSingleton<IAdaptivePool<T>>` resolving via `GetRequiredKeyedService<IAdaptivePool<T>>(string.Empty)` so single-pool apps can inject `IAdaptivePool<T>` directly without keyed-services awareness.
...
dotnet test --project tests/Oragon.AdaptivePool.Core.Tests --no-build -c Release
...
**1. [Rule 1 — Bug] PublicAPI.Unshipped.txt extra declaration `AdaptivePoolOptions<T>.AdaptivePoolOptions() -> void`**
- **Found during:** Task 3 build with PublicApiAnalyzers.
- **Issue:** The plan's expected-entries snippet listed an explicit parameterless constructor entry for the sealed record. With C# 12 `sealed record AdaptivePoolOptions<T>` defined as `{ }` body (no positional record parameters; only `init`-style properties), the analyzer does NOT surface a separate "implicit constructor" symbol that needs to be declared. Including the line caused `RS0017: Symbol ... is part of the declared API, but is either not public or could not be found` (× 3 TFMs).
- **Fix:** Removed the line `Oragon.AdaptivePool.Core.Builder.AdaptivePoolOptions<T>.AdaptivePoolOptions() -> void` from PublicAPI.Unshipped.txt. All other 75 entries match what the analyzer expects.
- **Files modified:** `src/Oragon.AdaptivePool.Core/PublicAPI.Unshipped.txt`.
...
```csharp
// Single-pool app:
services.AddAdaptivePool<IConnection>(string.Empty, b =>
...
var pool = sp.GetRequiredService<IAdaptivePool<IConnection>>();          // works
var pool = sp.GetRequiredKeyedService<IAdaptivePool<IConnection>>("");   // also works
```

```csharp
// Multi-pool app (same T, different names):
services.AddAdaptivePool<IConnection>("primary", b => b.Factory(...).WithBounds(2,10,2));
services.AddAdaptivePool<IConnection>("backup", b => b.Factory(...).WithBounds(1,5,1));

// Must use keyed resolve:
var primary = sp.GetRequiredKeyedService<IAdaptivePool<IConnection>>("primary");
var backup  = sp.GetRequiredKeyedService<IAdaptivePool<IConnection>>("backup");
```
...
If a future Phase 2 / Phase 4 requires guaranteed host integration, we can switch to a hard reference behind a separate `Oragon.AdaptivePool.Hosting` companion package — but the Core stays primitive.
...
1. **MaxSize=1 ping-pong stress test in `Oragon.AdaptivePool.Core.Stress`** (success criterion #2 from Phase 1 ROADMAP). This is the test-anchor for waiter-queue + counter rollback + cancellation correctness.
...
| 2    | fdb29d9 | feat(02-02): engine internals — sealed AdaptivePool with Channel direct-handoff waiter, PoolItem wrapper, telemetry, source-gen logging |
| 3    | db7ae8a | feat(02-03): DI extension AddAdaptivePool<T> + PublicAPI.Unshipped baseline (76 entries) + green build |
...
- `dotnet build Oragon.AdaptivePool.sln -c Release` exits 0; only the 6 SourceLink "no remote" warnings present (out-of-scope per Plan 01 SUMMARY).
- `dotnet test --project tests/Oragon.AdaptivePool.Core.Tests --no-build -c Release` → 3 passed, 0 failed (Plan 01 placeholder smoke test still green across net8/net9/net10).
