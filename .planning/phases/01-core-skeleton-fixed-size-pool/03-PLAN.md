...
must_haves:
  truths:
    ...
    - "Coverage of `Oragon.ElasticPool.Core` ≥ 90% line coverage (gated in CI for Core.Tests run only — Stress remains opt-in)"
    - "CI workflow .github/workflows/build.yml additionally runs the coverage gate; build fails if Core line coverage < 90%"
  artifacts:
    - path: tests/Oragon.ElasticPool.Core.Tests/Builder/BuilderValidationTests.cs
      provides: "Tests for Builder.Build() validation: missing Factory, invalid bounds matrix"
      contains: "[Fact]"
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/AcquireAndReturnTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/PoolItemDisposeTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/FactoryFailureTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/BeforeUseUnhealthyTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/WaitBehaviorTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/WarmupAndBoundsTests.cs
      provides: "Tests for ReadyAsync awaitability, Available == InitialSize after warmup, bounds enforcement at runtime"
      contains: "ReadyAsync"
    - path: tests/Oragon.ElasticPool.Core.Tests/Pool/DisposeDrainTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/DependencyInjection/ServiceCollectionExtensionsTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/Telemetry/MeterAndCounterTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Tests/TimeProvider/TimeProviderInjectionTests.cs
    ...
    - path: tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs
  ...
  key_links:
    - from: "tests/Oragon.ElasticPool.Core.Tests/Telemetry/MeterAndCounterTests.cs"
    ...
    - from: "tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs"
    ...
    - from: ".github/workflows/build.yml"
      to: "tests/Oragon.ElasticPool.Core.Tests"
...
Common test helper: define a small reusable `Resource` POCO inside each test file (or move to a shared file `tests/Oragon.ElasticPool.Core.Tests/TestSupport/Resource.cs` if duplication becomes annoying — the executor can decide, but keep tests self-contained for clarity).
...
Build hygiene reminder: every test must `await` returned ValueTask exactly once (PITFALLS Pitfall 1).
  ...
  <verify>
    <automated>cd /mnt/p/dynamic-pool && find tests/Oragon.ElasticPool.Core.Tests -name '*.cs' -type f | wc -l | awk '{ if ($1 < 12) { print "MISSING-TESTS:" $1; exit 1 } else print "TEST-FILES-OK:" $1 }' && grep -r 'using AwesomeAssertions' tests/Oragon.ElasticPool.Core.Tests/ | wc -l | awk '{ if ($1 < 11) { print "MISSING-AWESOMEASSERTIONS-IMPORTS:" $1; exit 1 } else print "AA-IMPORTS-OK:" $1 }' && ! grep -rE 'using FluentAssertions' tests/Oragon.ElasticPool.Core.Tests/ && grep -r 'MetricCollector' tests/Oragon.ElasticPool.Core.Tests/Telemetry/ && grep -r 'FakeTimeProvider' tests/Oragon.ElasticPool.Core.Tests/TimeProvider/ && grep -r 'FailureKind.FactoryThrew' tests/Oragon.ElasticPool.Core.Tests/Pool/FactoryFailureTests.cs && grep -r 'FailureKind.BeforeUseUnhealthy' tests/Oragon.ElasticPool.Core.Tests/Pool/BeforeUseUnhealthyTests.cs && dotnet test tests/Oragon.ElasticPool.Core.Tests -c Release 2>&1 | tee /tmp/test-task1.log && grep -qE '(Failed:[ ]*0|Failed!:[ ]*0|Passed!? -)' /tmp/test-task1.log && ! grep -qE 'Failed:[ ]*[1-9]' /tmp/test-task1.log && echo TASK1-DONE</automated>
  </verify>
  <done>11 unit test files exist (plus updated placeholder). All use AwesomeAssertions (no FluentAssertions imports). Every must-have truth has at least one corresponding test. `dotnet test tests/Oragon.ElasticPool.Core.Tests -c Release` runs and all tests pass under MTP runner. Failing tests trigger CI failure.</done>
</task>
...
Run the stress test locally and confirm green. Note: This test is NOT in the CI default sweep (Plan 01 build.yml does not invoke the Stress project). It is a manual-execution / nightly-job (Phase 4) gate.
  ...
  <verify>
    <automated>cd /mnt/p/dynamic-pool && test -f tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && grep -q 'MaxSize1_HundredsOfThreads_TenThousandIterations_NoDeadlock' tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && grep -q 'Channel<TaskCompletionSource\|direct-handoff' tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && grep -q 'WithBounds(minSize: 1, maxSize: 1, initialSize: 1)' tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && dotnet test tests/Oragon.ElasticPool.Core.Stress -c Release 2>&1 | tee /tmp/stress-test.log && grep -qE '(Failed:[ ]*0|Passed!? -.*Failed:[ ]*0)' /tmp/stress-test.log && ! grep -qE 'Failed:[ ]*[1-9]' /tmp/stress-test.log && echo STRESS-DONE</antomated>
    <automated>cd /mnt/p/dynamic-pool && test -f tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && grep -q 'MaxSize1_HundredsOfThreads_TenThousandIterations_NoDeadlock' tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && grep -q 'WithBounds(minSize: 1, maxSize: 1, initialSize: 1)' tests/Oragon.ElasticPool.Core.Stress/PingPongStressTest.cs && dotnet test tests/Oragon.ElasticPool.Core.Stress -c Release 2>&1 | tee /tmp/stress-test.log && ! grep -qE 'Failed:[ ]*[1-9]' /tmp/stress-test.log && echo STRESS-DONE</automated>
  </verify>
  <done>PingPongStressTest.cs exists with the 256-thread × 40-iteration MaxSize=1 ping-pong workload; 25s logical watchdog + 5s per-call timeout. Final invariants check `InUse==0` and `Available==1`. `dotnet test tests/Oragon.ElasticPool.Core.Stress` passes within 60s. Placeholder is reduced to comment-only or deleted. Stress is still NOT in the CI default sweep.</done>
</task>
    ...
    steps:
      ...
      - name: enforce 90% line coverage on Core
        ...
        run: |
          ...
          # Generate text summary, restricted to Oragon.ElasticPool.Core assembly
          reportgenerator \
            -reports:"$REPORTS" \
            -targetdir:"./TestResults/${{ matrix.tfm }}/coverage-report" \
            -reporttypes:TextSummary \
            -assemblyfilters:"+Oragon.ElasticPool.Core"
          ...
          echo "Line coverage on Oragon.ElasticPool.Core: $LINE_PCT%"
...
- `assemblyfilters:"+Oragon.ElasticPool.Core"` restricts measurement to the production library — test code is excluded from the % calculation.
...
```bash
dotnet test tests/Oragon.ElasticPool.Core.Tests --collect:"XPlat Code Coverage" --results-directory ./TestResults
reportgenerator -reports:./TestResults/**/coverage.cobertura.xml -targetdir:./TestResults/coverage -reporttypes:TextSummary -assemblyfilters:"+Oragon.ElasticPool.Core"
cat ./TestResults/coverage/Summary.txt
```
Confirm the "Line coverage" line shows ≥ 90.0%.
  </action>
  <verify>
    <automated>cd /mnt/p/dynamic-pool && grep -q 'XPlat Code Coverage' .github/workflows/build.yml && grep -q 'reportgenerator' .github/workflows/build.yml && grep -q '+Oragon.ElasticPool.Core' .github/workflows/build.yml && grep -q '90' .github/workflows/build.yml && ! grep -q 'Oragon.ElasticPool.Core.Stress' .github/workflows/build.yml && dotnet test tests/Oragon.ElasticPool.Core.Tests --collect:"XPlat Code Coverage" --results-directory ./TestResults -c Release 2>&1 | tee /tmp/cov-test.log && (which reportgenerator > /dev/null 2>&1 || dotnet tool install -g dotnet-reportgenerator-globaltool) && export PATH="$PATH:$HOME/.dotnet/tools" && REPORTS=$(find ./TestResults -name 'coverage.cobertura.xml' -print | tr '\n' ';' | sed 's/;$//') && reportgenerator -reports:"$REPORTS" -targetdir:./TestResults/coverage-report -reporttypes:TextSummary -assemblyfilters:"+Oragon.ElasticPool.Core" && LINE_PCT=$(grep -E '^[[:space:]]*Line coverage:' ./TestResults/coverage-report/Summary.txt | grep -oE '[0-9]+(\.[0-9]+)?' | head -1) && awk -v cov="$LINE_PCT" 'BEGIN { exit (cov+0 >= 90.0 ? 0 : 1) }' && echo "COVERAGE-OK ${LINE_PCT}%"</automated>
  </verify>
  <done>build.yml updated with coverage collection step + reportgenerator install + 90% line-coverage gate restricted to `+Oragon.ElasticPool.Core`. Stress project still not invoked. Local `dotnet test --collect:"XPlat Code Coverage"` followed by reportgenerator confirms ≥90% line coverage on Core. Test artifacts uploaded on every CI run for diagnosability.</done>
</task>
...
dotnet test tests/Oragon.ElasticPool.Core.Tests -c Release --no-build             # all pass
dotnet test tests/Oragon.ElasticPool.Core.Tests --collect:"XPlat Code Coverage" \
...
# Manual stress run
dotnet test tests/Oragon.ElasticPool.Core.Stress -c Release --no-build            # passes within 60s
```

Spot-check:
- `grep -r 'using FluentAssertions' tests/` returns nothing
- `find tests/Oragon.ElasticPool.Core.Tests -name '*.cs' | wc -l` >= 12
- `grep -c '\[Fact\]\|\[Theory\]' tests/Oragon.ElasticPool.Core.Tests/**/*.cs` >= 30 (well above the success-criterion-coverage minimum)
...
- [ ] `dotnet test tests/Oragon.ElasticPool.Core.Tests` reports 0 failed
...
- [ ] `dotnet test tests/Oragon.ElasticPool.Core.Stress` reports 0 failed and completes within 60s
- [ ] Coverage of `+Oragon.ElasticPool.Core` ≥ 90.0% line coverage (verified locally via reportgenerator)
...
- The CI coverage workflow design (XPlat collector → reportgenerator → 90% gate restricted to `+Oragon.ElasticPool.Core`)
