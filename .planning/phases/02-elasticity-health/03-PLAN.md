...
must_haves:
  truths:
    ...
    - "ActivityListener attached to 'Oragon.ElasticPool' captures Pool.Acquire, Pool.Release, Pool.HealthCheck, Pool.Grow, Pool.Shrink, Pool.Sweep spans across their respective happy-path tests."
...
ActivitySource name: "Oragon.ElasticPool"
Meter name: "Oragon.ElasticPool"
...
- `Grow_RecordsCounterAndSpan`: attach `MetricCollector<long>` on `pool.grow.count` + `CapturedActivities("Oragon.ElasticPool")`; trigger one grow; assert exactly 1 counter increment + exactly 1 `Pool.Grow` stopped activity with the expected tags.
...
- `GrowCount_IncrementsExactlyOncePerGrow`: `MetricCollector<long>(meterFactory, "Oragon.ElasticPool", "pool.grow.count", TimeProvider.System)`; trigger 3 grows; assert snapshot has 3 measurements summing to 3 with `pool.name` tag.
...
public async Task BurstIdleBurst_PoolGrowsShrinksGrowsAgain_WithoutDeadlocks()
{
    ...
    using var growCounter = new MetricCollector<long>(
        ...
        "Oragon.ElasticPool", "pool.grow.count", TimeProvider.System);
    using var shrinkCounter = new MetricCollector<long>(
        sp.GetRequiredService<IMeterFactory>(),
        "Oragon.ElasticPool", "pool.shrink.count", TimeProvider.System);

    var poolName = "burst-stress";
    var pool = (ElasticPool<Resource>)ElasticObjectPoolFactory.Build<Resource>(sp)
...
- If coverage < 90%, identify gap-files and add closing tests in the lowest-coverage Phase 2 component file (likely `BackgroundSweeper.cs` or `ElasticPool.cs`'s grow path). Closing tests follow the Phase 1 pattern of using only the public surface — no `[ExcludeFromCodeCoverage]` markers.
