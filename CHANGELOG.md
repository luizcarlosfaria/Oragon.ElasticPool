...
- **Live pool state API and gauges.** `IAdaptivePool<T>` now exposes `Total` and `Waiting` alongside `Available` and `InUse`. The Core meter now publishes observable gauges `pool.size`, `pool.available`, `pool.in_use`, and `pool.waiting`, all tagged with `pool.name`.
